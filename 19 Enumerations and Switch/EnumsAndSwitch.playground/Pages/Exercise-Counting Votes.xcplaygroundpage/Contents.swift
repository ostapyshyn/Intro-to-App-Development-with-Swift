...
for choice in tripDestinationVotes {
    switch choice {
    case .beach:
        beach += 1
...
if beach > factory {
    print("Result is \(beach) for BEACH")
} else {
    print("Result is \(factory) for FACTORY")
}
...
// they have different types , in previous similar exercise they were both Bool, but here we have two different types SchoolMascotOption and ClassTripDestination, and i
// don't know how to combine them, ANY doesn't work, enum aswell
...
if salamander > marmot && salamander > neither {
    print("Result is \(salamander) for Salamander")
} else if marmot > salamander && marmot > neither {
    print("Result is \(salamander) for Salamander")    
} else {
    print("NEITHER")
}
